...
struct ContentView: View {
    ...
    // Helper para elegir tamaño y peso
    func fontStyle(for line: Int, index: Int) -> Font {
        if index == line {
            return .system(size: 64, weight: .bold, design: .default)
        } else {
            return .system(size: 56, weight: .light, design: .default)
        }
    }
    
    var body: some View {
        TimelineView(.everyMinute) { context in
            let lang = languageCode //locale.language.languageCode?.identifier ?? "en"
            ...
            VStack(alignment: .center) {
                ...
                /*ThreeLineSizedText(
                    arriba:  "casi las siete",
                    medio:   "menos",
                    abajo:   "cinco",
                    ihora:   1,     // 1, 2 o 3
                    maxFontSize: 120
                )*/
                .frame(maxWidth: .infinity,            // Ocupa todo el ancho posible del widget
                       alignment: .center)            // y alinea el VStack al centro
                .padding()
                .onTapGesture {
                    // 1️⃣ Cambiamos el idioma
                    if let idx = languages.firstIndex(of: languageCode) {
                        languageCode = languages[(idx + 1) % languages.count]
                    } else {
                        languageCode = languages[0]
                    }
                    // 2️⃣ Recargamos la complicación
                    WidgetCenter.shared.reloadAllTimelines()
                }
            }
            // ← aquí forzamos “remontar” el TimelineView cuando cambie el idioma
            .id(languageCode)
        }
    }
}
